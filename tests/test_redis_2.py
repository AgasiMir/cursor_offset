...
class TestRedisManager:
    ...
    async def test_connect_success(self, mock_redis, redis_manager):
        ...
        mock_log.info.assert_any_call("Начинаю подключение к Redis host=localhost, port=6379...")
        mock_log.info.assert_any_call("Успешное подключение к Redis host=localhost, port=6379")
    ...
    async def test_set_key_with_expire(self, redis_manager):
        ...
        # Проверка
        redis_manager.redis.set.assert_awaited_once_with("test_key", "test_value", ex=60)
