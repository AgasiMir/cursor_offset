...
async def setup_database(check_test_mode):
    ...
    Поведение:
        ...
        - Использует `engine_null_pull` — специальный движок без пула соединений,
          подходящий для DDL-операций.
        - Выполняет:
    ...
    Используется в:
        Интеграционных и функциональных тестах, где требуется работа с реальной (обычно SQLite или
        тестовой PostgreSQL) БД.

    Почему это важно:
        - Гарантирует, что каждый тест начинается с пустой базы.
    ...
    Примечания:
        ...
        - `engine_null_pull` должен быть сконфигурирован отдельно
           (например, через `create_async_engine(url, poolclass=NullPool)`).
    ...
    Внимание:
        ...
        Если нужно исключить очистку БД в каком-то тесте — пересмотрите архитектуру или
        используйте отдельную метку.
    """
    ...
    with open(r"tests/mock_posts.json", encoding="utf-8") as file_posts:
